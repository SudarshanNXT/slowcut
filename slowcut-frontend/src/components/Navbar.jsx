...
import { IoFlash } from "react-icons/io5"; // Thunder/lightning icon for Activity
import { FaFire } from "react-icons/fa"; // Fire icon for Trending
...
import logo from "../assets/slowcut-brand.png"
const Navbar = () => {
        ...
        <>
                            ...
                            <>
                                <div className='relative flex items-center group font-bold rounded-t-md hover:bg-light w-fit p-2'>
                                    ...
                                    <div className='absolute left-0 top-full hidden group-hover:flex flex-col bg-inherit text-black rounded-b-md w-full font-normal text-xs z-50'>
                                        ...
                                        <a href={`/${sessionUsername}/network`} className='pl-2 py-2 hover:bg-gray-700 hover:text-white'>Network</a>
                                ...
                                <Link className='flex items-center hover:text-white' to={'/activity'}><AiTwotoneThunderbolt /></Link>
                                <Link className='flex items-center hover:text-white' to={'/trending'}>Trending</Link>
                                <Link className='flex items-center hover:text-white' to={'/members'}>Members</Link>
                                <Link className='flex items-center hover:text-white' to={'/films'}>Films</Link>
                                <Link className='flex items-center hover:text-white' to={'/news'}>News</Link>
                                <Link className='flex items-center hover:text-white' to={'/ai'}><RiRobot3Fill /></Link>
                                ...
                                <div className='inline-flex items-center font-bold h-[28px] bg-hover rounded-md relative'>
                                    <button onClick={() => setSuperMegaForm(true)} className=' hover:bg-green-500 flex items-center uppercase p-1 rounded-l-md text-white'><FaPlus className='mr-1' size={12}/><div className='mr-1'>Log</div></button>
            ...
            <nav className={`flex md:hidden text-gray-300 bg-primary h-[65px] uppercase z-50 w-full relative`}>
                ...
                <div className={`absolute top-full w-full flex flex-col bg-primary transform transition-transform duration-300 font-bold z-10 
                    ...
                    {isLoggedIn ? (
                        <>
                            ...
                            <div className='grid grid-cols-2 gap-4 pl-8 pr-2 font-normal text-gray-400 mb-1'>
                                ...
                                <a className='hover:bg-gray-700 hover:text-white' href={`/${sessionUsername}/network`} onClick={() => setMenuDropdown(false)}>Network</a>
                    ...
                    <Link to={'/activity'} onClick={() => setMenuDropdown(false)} className='flex items-center pl-4 py-2 hover:bg-gray-700'><IoFlash className='mr-3'/> <span><AiTwotoneThunderbolt /></span></Link>
                    <Link to={'/trending'} onClick={() => setMenuDropdown(false)} className='flex items-center pl-4 py-2 hover:bg-gray-700'><FaFire className='mr-3'/> <span>TRENDING</span></Link>
                    <Link to={'/members'} onClick={() => setMenuDropdown(false)} className='flex items-center pl-4 py-2 hover:bg-gray-700'><FaUsers className='mr-3'/> <span>MEMBERS</span></Link>
                    ...
                    <Link to={'/news'} onClick={() => setMenuDropdown(false)} className='flex items-center pl-4 py-2 hover:bg-gray-700'><FaNewspaper className='mr-3'/> <span>NEWS</span></Link>
                    <Link to={'/ai'} onClick={() => setMenuDropdown(false)} className='flex items-center pl-4 py-2 hover:bg-gray-700'><FaInfoCircle className='mr-3'/> <span><RiRobot3Fill /></span></Link>
