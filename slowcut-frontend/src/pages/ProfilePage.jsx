...
const ProfilePage = () => {
    ...
    useEffect(() => {
        ...
        getProfileData();
    }, [username, update, apiBaseUrl, navigate, sessionId]);
            ...
            <div>
                ...
                <div className='flex flex-col md:items-start space-y-6 md:space-y-0 lg:flex-row lg:space-x-8'>
                    <div className='flex flex-col w-full px-3 md:px-0 space-y-6'>
                        <FavoriteFilms 
                            authorized={authorized} 
                            favorite_films={profileData.favorite_films} 
                            setUpdate={setUpdate} 
                                ...
                                profileData.diary.map((month, index) => (
                                    <div className='flex space-x-4 border-b border-b-gray-600 pb-2' key={index}>
                                        <div className='relative w-[40px] h-[40px] flex-shrink-0'>
                                            ...
                                            <span className="absolute inset-0 flex items-center justify-center text-xs font-semibold text-gray-700 mt-1">
                                        ...
                                        <div className='flex flex-col space-y-1'>
                                            {month.map((entry, index) => (
                                                <div className='flex items-end space-x-2' key={index}>
                                                    ...
                                                    <Link to={`/film/${entry.movie.id}`} className='text-gray-300 text-sm hover:text-blue-500 font-semibold line-clamp-1'>
                                                        {entry.movie.title}
                                                    </Link>
                                                </div>
                                            ))}
